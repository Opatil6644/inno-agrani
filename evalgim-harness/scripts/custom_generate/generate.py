...
class DiffusersPipeline(ImageGenerationPipeline):
    def __init__(self, model_id: str, cfg_scale: float, model_commit_id: str = None, device: int | str = "balanced"):
        self.model_id = model_id
        self.accelerator = Accelerator(mixed_precision="no")
        self.pipeline = DiffusionPipeline.from_pretrained(
            model_id, torch_dtype=torch.float16, device_map=device, revision=model_commit_id
        )
...
def get_model_details_from_csv(model_name, config_path=None):
    # Get the directory where generate.py lives
    current_dir = Path(__file__).resolve().parent 
    # Navigate to ../../config/model_details.csv
    DEFAULT_CSV_PATH = current_dir.parent.parent / "config" / "model_details.csv"
...
def main(args: argparse.Namespace, data_file: Path):
    ...
    # 1. Fetch the model_commit_id
    model_commit_id = None
    if args.model_csv:
        details = get_model_details_from_csv(args.model_id, args.model_csv)
        model_commit_id = details['model_commit_id']

    pipeline = DiffusersPipeline(args.model_id, args.cfg_scale, model_commit_id=model_commit_id)
    ...
    latency_output_json = args.output_path + "/latency.json"
    throughput_output_json = args.output_path + "/throughput.json"

    with open(latency_output_json, "w") as f:
        json.dump(results_latency, f, indent=4)

    with open(throughput_output_json, "w") as f:
        json.dump(results_throughput, f, indent=4)
...
def parse_args():
    ...
    parser.add_argument("--model_csv", type=str, default="config/model_details.csv", help="Path to CSV with model and commit_id")
