#!/bin/bash
#################################################################################
# PURPOSE: Automated Model Inference and NVIDIA Profiling Pipeline
# -------------------------------------------------------------------------------
# This script automates the process of evaluating multiple models (from TIMM) 
# by performing the following steps for each model:
# 
# 1. ENVIRONMENT SETUP: Loads model names from a master list and validates paths.
# 2. INFERENCE & PROFILING: Executes model inference while running 'nsys profile'
#    to capture CUDA, cuBLAS, and NVTX traces.
# 3. KERNEL ANALYSIS: Captures cuBLAS logs to extract GEMM dimensions (m, n, k) 
#    and classifies GPU kernels as open-source or closed-source.
# 4. DATA AGGREGATION: Post-processes raw profiling data into structured 
#    CSV/XLSX reports (MNK frequencies, NVJET details, and kernel summaries).
# 5. CLEANUP: Automatically removes heavy temporary files (.nsys-rep, .sqlite) 
#    to save disk space after extraction is complete.
#################################################################################
...
# Default image path (used when img_path is not provided in config.json)
DEFAULT_IMG_PATH="config/kitten.jpg"

# -------------------------------
# Read img_path from config.json
IMG_PATH=$(jq -r '.img_path // empty' "$CONFIG_JSON_PATH")

# -------------------------------
# If img_path is empty or not provided,
# fall back to the default image path
# -------------------------------
if [ -z "$IMG_PATH" ]; then
    IMG_PATH="$DEFAULT_IMG_PATH"
    echo "img_path not provided in config.json. Using default image: $IMG_PATH"
else
    echo "Using custom image path from config.json: $IMG_PATH"
fi
...
for MODEL_NAME in "${MODEL_NAMES_ARRAY[@]}"; do
  ...
  nsys profile --trace=cuda,nvtx,osrt,cudnn,cublas --force-overwrite true --output $MODEL_PROF_RESULTS_PATH/$REPORT_FILENAME  python evaluate/timm_run.py \
