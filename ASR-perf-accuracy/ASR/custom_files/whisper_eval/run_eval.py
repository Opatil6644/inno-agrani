...
def main(args):
    if args.model_id == "openai/whisper-small.en":
        revision = "e8727524f962ee844a7319d92be39ac1bd25655a"
    else:
        revision = "06f233fe06e710322aca913c1bc4249a0d71fce1"

    config = AutoConfig.from_pretrained(args.model_id, revision=revision)
    cls_model = AutoModelForSpeechSeq2Seq if type(config) in MODEL_FOR_SPEECH_SEQ_2_SEQ_MAPPING else AutoModelForCTC
    model = cls_model.from_pretrained(args.model_id, torch_dtype=torch.bfloat16, attn_implementation="sdpa", revision=revision).to(args.device)
