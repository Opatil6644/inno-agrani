...
# ==============================================================================
# PURPOSE: Canary ASR Profiling & Kernel Analysis Worker
# ------------------------------------------------------------------------------
# This script performs deep performance profiling for Canary-based models 
# (specifically SALM/Qwen variants).
#
# Key Functions:
...
MODEL_IDs=("nvidia/canary-qwen-2.5b")
BATCH_SIZE=192
DEVICE_ID=0

# Defined these AT THE TOP so the loop can see them
#RESULTS_PATH=$(pwd)
...
for MODEL_ID in "${MODEL_IDs[@]}"; do
    # Create a safe name (replace / with --)
    ...
    for ds in "${DATASETS_TO_RUN[@]}"; do
        ...
        echo "Generating stats and kernel reports..."
        
        # Using the full path with the extension for nsys stats
