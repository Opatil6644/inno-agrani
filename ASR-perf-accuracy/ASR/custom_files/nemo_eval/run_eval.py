...
def load_asr_model(model_id: str, revision: str, device: str):
    # HF cache lives in ~/.cache/huggingface/hub by default
    # If already downloaded at this exact revision, it's a no-op
    model_dir = snapshot_download(
        repo_id=model_id,
        revision=revision
        # No local_dir — let HF manage its own cache
    )

    nemo_files = [f for f in os.listdir(model_dir) if f.endswith(".nemo")]
    if not nemo_files:
        raise FileNotFoundError(f"No .nemo file found in {model_dir}")

    return ASRModel.restore_from(
        restore_path=os.path.join(model_dir, nemo_files[0]),
        map_location=device
    )



def get_model(model_id: str, device):
    MODELS = {
            "nvidia/parakeet-tdt-0.6b-v3": {
                "revision": "6d590f77001d318fb17a0b5bf7ee329a91b52598",
            },
            "nvidia/canary-qwen-2.5b": {
                "revision": "6cfc37ec7edc35a0545c403f551ecdfa28133d72",
            }
        }
    ...
    return load_asr_model(model_id=model_id, revision=revision, device=device)
...
def main(args):
    ...
    if args.model_id.endswith(".nemo"):
        ...
        # args.model_id passed directly
        model = get_model(args.model_id, device)
