...
def main():
    ...
    for f in excel_files:
        print(f"Reading: {f}")
        try:
            # Use ExcelFile to inspect the file without crashing
            xl = pd.ExcelFile(f)
            sheet_names = xl.sheet_names
            num_sheets = len(sheet_names)

            # Process the first sheet if it exists
            if num_sheets >= 1:
                df1 = xl.parse(0)
                if not df1.empty:
                    sheet1_list.append(df1)
            
            # Process the second sheet ONLY if it exists
            if num_sheets >= 2:
                df2 = xl.parse(1)
                if not df2.empty:
                    sheet2_list.append(df2)
            else:
                print(f"  -> Note: Only {num_sheets} sheet(s) found. Skipping second sheet search.")
